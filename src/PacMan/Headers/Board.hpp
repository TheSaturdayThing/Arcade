...
void Board::ResetPosition(Entity &mEntity){
	...
	for(unsigned short i = 0; i < BoardHeight * BoardWidth; i++){
		...
		if(CharBoard[i] == '0' && mEntity.GetIdentity() == EntityType::ePacMan){
			mEntity.ModX((WindowWidth/2 - BoardWidth*BlockSize24/2) +x * BlockSize24 + BlockSize24/2);
...
void Board::Draw(unsigned char ActualMap[], Timer MapAnimationTimer){
	ScoreWordTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) );
	ScoreTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) , BlockSize32);
	HighScoreWordTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) +336);
	HighScoreTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) +336, BlockSize32);
	MapTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2));
	for(unsigned char i = 1; i <= Lives; i++){
		LivesTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) +i * BlockSize32, 26 * BlockSize32 - BlockSize32/4);
	}
	if(!MapAnimationTimer.isStarted()){
		DoorTexture.render(WindowWidth/2 - 23, WindowHeight/2 - 165);
		char y = -1;
		for(unsigned short i = 0; i < BoardHeight * BoardWidth; i++){
			unsigned char x = i % BoardWidth;
			if(x == 0)
			...
			if(ActualMap[i] == BlockType::Pellet)
				PelletTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) +x * BlockSize24, y * BlockSize24);
			if(ActualMap[i] == BlockType::Energizer)
				EnergizerTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) +x * BlockSize24, y * BlockSize24);
		}
	}
	else{	
