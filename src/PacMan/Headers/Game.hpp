...
void Game::Start(){
	...
	Ready.render((WindowWidth/2 - BoardWidth*BlockSize24/2) + 11 * BlockSize24, 20 * BlockSize24 - 5);
}
...
void Game::Draw(){
	...
	if(!IsGameStarted){
		GameOverTexture.render((WindowWidth/2 - BoardWidth*BlockSize24/2) + 9 * BlockSize24, 20 * BlockSize24 - 5);
		return;
	}
	mFruit.Draw();
	if(!MapAnimationTimer.isStarted()){
