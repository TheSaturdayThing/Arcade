...
Ghost::Ghost(SDL_Color MyColor, EntityType MyIdentity) : Entity(MyIdentity){
	...
	CanUseDoor = false;
	Status = false;
	DoorTarget.ModCoords((WindowWidth/2 - BoardWidth*BlockSize24/2) + 13 * BlockSize24 + BlockSize24 / 2, 15 * BlockSize24);
}
...
bool Ghost::IsHome(){
	if(this->GetX() > (WindowWidth/2 - BoardWidth*BlockSize24/2) +11 * BlockSize24 && this->GetX() < (WindowWidth/2 - BoardWidth*BlockSize24/2) +17 * BlockSize24){
