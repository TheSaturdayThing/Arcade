...
unsigned char Pac::FoodCollision(unsigned char ActualMap[]){  //bool
	float cell_x = (this->GetX() - (WindowWidth/2 - BoardWidth*BlockSize24/2)) / static_cast<float>(BlockSize24);
	float cell_y = this->GetY() / static_cast<float>(BlockSize24);  //posizione attuale in board
	Position BoardPos;
	for(unsigned char SideDir = 0; SideDir < 4; SideDir++){ // check su tutti i 4 lati della mia entità -> basterebbe farlo solo sulla direzione
