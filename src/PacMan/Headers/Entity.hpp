...
bool Entity::WallCollision(short x, short y, unsigned char ActualMap[], bool CanUseDoor){
	float cell_x = (x - (WindowWidth/2 - BoardWidth*BlockSize24/2)) / static_cast<float>(BlockSize24);
...
void Entity::CheckWrap(){
	if(this->GetX() > WindowWidth/2 + (BoardWidth*BlockSize24)/2 + BlockSize24)
		this->ModX(WindowWidth/2 - (BoardWidth*BlockSize24)/2 - BlockSize24);
	if(this->GetX() < WindowWidth/2 - (BoardWidth*BlockSize24)/2 - BlockSize24)
		this->ModX(WindowWidth/2 + BoardWidth*BlockSize24/2 + BlockSize24);
}
